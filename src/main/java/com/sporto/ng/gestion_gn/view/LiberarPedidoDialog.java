...
public class LiberarPedidoDialog extends JDialog {
	...
	public LiberarPedidoDialog(ProductoDao productoDao, PedidoDao pedidoDao,MovimientoCajaDao movimientoCajaDao
		...
		panelCliente.setBorder(new TitledBorder(null, "CLIENTE", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		...
		agregarStockBtn_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				MovimientoCaja unMovimientoPago = MovimientoCaja.builder()
							.cliente(cliente)
							.comentario(pagoComentario.getText())
							.fecha(new Date())
		...
		panelPedidos.setBorder(new TitledBorder(null, "PEDIDOS A LIBERAR", TitledBorder.LEADING, TitledBorder.TOP, null, null));
...
;
	...
	public void guardarMovimientos() {
		...
		if(estadoCC < limiteDeuda) {
			JOptionPane.showMessageDialog(this, "El cliente supera su liminte de descubierto.", "Error", JOptionPane.ERROR_MESSAGE);
			return;
			
		}
		
		registrarIngresos(tablePagos);
	...
	private void registrarIngresos(PagoTable table) {
